...
const screens = {
  login: {
    ...
    subtitle: "Sua próxima refeição favorita começa aqui.",
    action: "Entrar",
  },
  register: {
    title: "Crie sua conta",
    subtitle: "Seu próximo pedido começa por aqui.",
    action: "Criar conta",
  },
  recover: {
    title: "Esqueceu a senha?",
    subtitle: "Informe o e-mail da sua conta para recuperar seu acesso.",
    action: "Recuperar senha",
  },
  verify: {
    title: "Confira seu e-mail",
    subtitle: "Digite o código de 6 números que enviamos para você.",
    action: "Confirmar código",
  },
};
const readMode = () =>
...
function GoogleIcon() {
  return (
    ...
    >
      <path
        fill="#4285F4"
        d="M43.61 24.46c0-1.36-.12-2.66-.35-3.92H24v7.41h11a9.4 9.4 0 0 1-4.08 6.18v5.14h6.61c3.87-3.56 6.08-8.8 6.08-14.81Z"
      />
      <path
        fill="#34A853"
        d="M24 44c5.52 0 10.15-1.83 13.53-4.97l-6.61-5.14c-1.83 1.23-4.18 1.98-6.92 1.98-5.32 0-9.85-3.59-11.47-8.42H5.71v5.29A20 20 0 0 0 24 44Z"
      />
      <path
        fill="#FBBC05"
        d="M12.53 27.45a12 12 0 0 1 0-6.9v-5.29H5.71a20 20 0 0 0 0 17.48l6.82-5.29Z"
      />
      <path
        fill="#EA4335"
        d="M24 12.13c3.01 0 5.69 1.04 7.82 3.09l5.86-5.86A19.65 19.65 0 0 0 24 4 20 20 0 0 0 5.71 15.26l6.82 5.29c1.62-4.83 6.15-8.42 11.47-8.42Z"
      />
    </svg>
  );
}
...
export function LoginPage() {
  ...
  const [pendingSignup, setPendingSignup] = useState(() => {
    try {
      return JSON.parse(sessionStorage.getItem(pendingSignupKey)) || null;
    } catch {
      return null;
    }
  });
  ...
  function update(field, value) {
    ...
    setErrors((previous) => {
      ...
      if (
        field === "password" &&
        (touched.confirmPassword || previous.confirmPassword)
      ) {
        if (validation.confirmPassword)
          updated.confirmPassword = validation.confirmPassword;
  ...
  async function submit(event) {
    ...
    if (!isSupabaseConfigured) {
      setNotice(
        "A conexão com o Supabase ainda precisa das chaves do projeto.",
      );
      return;
    }
  ...
  function handleOtpKeyDown(index, event) {
    if (event.key === "Backspace" && !otp[index] && index > 0) {
      ...
      setOtp((current) =>
        current.map((value, position) => (position === index - 1 ? "" : value)),
      );
    }
  ...
  function field(name, label, type, placeholder, icon, autoComplete, hint) {
    const validation = validateAuthForm(mode, values);
    const valid = Boolean(
      touched[name] && String(values[name]).trim() && !validation[name],
    );
    return (
      <Input
        key={mode + name}
        ...
        onBlur={() => blurField(name, type)}
        valid={valid}
        onKeyUp={
          type === "password"
            ? (event) => setCapsLock(event.getModifierState("CapsLock"))
            : undefined
        }
        onKeyDown={
          type === "password"
        ...
        maxLength={name === "cpf" ? 14 : undefined}
        error={errors[name]}
        hint={hint}
        spellCheck={false}
  ...
  return (
    <main className={styles.page}>
      <div className={styles.photo}>
        <img
          src={loginImage}
          alt="Tigre entregador em uma moto amarela pela cidade ao pôr do sol"
          fetchPriority="high"
        />
      </div>
        ...
        <div className={styles.content}>
          {(recover || verify) && (
            <a
              ...
              href={verify ? "#criar-conta" : "#entrar"}
            >
              <Icon icon={ArrowLeft} />
              {verify ? "Alterar e-mail" : "Voltar para o login"}
            </a>
          )}
          <header className={styles.header}>
            ...
            {verify && pendingSignup?.email && (
              <p className={styles.sentTo}>{pendingSignup.email}</p>
            )}
          </header>
          {verify ? (
            <form className={styles.otpForm} onSubmit={verifyCode} noValidate>
              <fieldset
                className={`${styles.otpFieldset} ${otpError ? styles.otpInvalid : ""} ${success ? styles.otpVerified : ""}`}
              >
              ...
              <Button
                ...
                success={success}
                loadingLabel={
                  success ? "Conta confirmada" : "Confirmando código"
                }
                className={`${styles.submit} ${success ? styles.successButton : ""}`}
              >
                {screen.action}
              ...
              <div className={styles.resendRow}>
                ...
                <button
                  type="button"
                  onClick={resendCode}
                  disabled={Boolean(resendIn) || loading}
                >
                  {resendIn
                    ? `Reenviar em 0:${String(resendIn).padStart(2, "0")}`
                    : "Enviar outro código"}
                </button>
              </div>
            </form>
            ...
            <form className={styles.form} onSubmit={submit} noValidate>
              ...
              {register &&
                field(
                  ...
                  IdCard,
                  "off",
                  "Seu CPF fica protegido e não aparece no catálogo.",
                )}
              {field(
                "email",
                "E-mail",
              ...
              {!recover &&
                field(
                  ...
                  "Senha",
                  "password",
                  register ? "Crie uma senha" : "Digite sua senha",
                  Lock,
                  register ? "new-password" : "current-password",
                  register ? "Use pelo menos 8 caracteres." : undefined,
                )}
              {register &&
                field(
                  "confirmPassword",
              ...
              {!register && !recover && (
                <div className={styles.forgotRow}>
                  <a className={styles.textLink} href="#recuperar-senha">
                    Esqueceu a senha?
                  </a>
                </div>
              )}
          ...
          {!recover && !verify && (
            <>
              ...
              >
                <GoogleIcon />
                {register ? "Continuar com o Google" : "Entrar com o Google"}
              </Button>
              <footer className={styles.footer}>
                ...
                <a
                  ...
                  href={register ? "#entrar" : "#criar-conta"}
                >
          ...
          {!verify && (
            <a className={styles.explore} href="#catalogo">
              Só quero dar uma olhada no cardápio <Icon icon={ArrowRight} />
            </a>
          )}
        </div>
      </section>
      <AnimatePresence>
        {loading && (
          <LoadingOverlay
            label={verify ? "Quase lá" : "Só um instante"}
            detail={
              verify
                ? "Conferindo seu código com segurança."
                : "Estamos cuidando do seu acesso."
            }
          />
        )}
      </AnimatePresence>
