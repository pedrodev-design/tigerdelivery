...
function HelpPage({ navigate }) {
  const shortcuts = [
    { id: 'home', icon: MagnifyingGlass, title: 'Explorar o catálogo', text: 'Encontre pratos, categorias e ofertas.' },
    { id: 'orders', icon: Receipt, title: 'Meus pedidos', text: 'Acompanhe cada etapa da entrega.' },
    { id: 'favorites', icon: Heart, title: 'Favoritos', text: 'Volte aos pratos que você salvou.' },
  ]
  return <motion.section className={styles.helpPage} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: .24, ease: [0.22, 1, 0.36, 1] }}>
    <section className={styles.helpHero}>
      <div><span>Ajuda TigreDelivery</span><h1>Precisou? Estamos aqui.</h1><p>Sobre seu pedido, seu endereço ou como começar.</p></div>
      <img src="/duvida-icon.png" alt="" />
    </section>
    <nav className={styles.helpShortcuts} aria-label="Atalhos da ajuda">{shortcuts.map(item => <button key={item.id} onClick={() => navigate(item.id)}><span><Icon icon={item.icon} /></span><div><strong>{item.title}</strong><small>{item.text}</small></div><Icon icon={CaretRight} /></button>)}</nav>
    <section className={styles.helpFaq}>
      <header><span>Bom saber</span><h2>As dúvidas mais comuns.</h2></header>
      <div>
        <details open><summary>Como faço um pedido?<Icon icon={Plus} /></summary><p>Escolha um prato, toque em adicionar e abra a sacola. Depois, confirme o endereço, a forma de recebimento e o pagamento.</p></details>
        <details><summary>Onde acompanho a entrega?<Icon icon={Plus} /></summary><p>Abra Meus pedidos para ver o status atualizado e, quando disponível, acompanhar o motorista no mapa.</p></details>
        <details><summary>Como altero o endereço?<Icon icon={Plus} /></summary><p>Toque no endereço exibido no topo da tela. Você pode buscar pelo CEP e ajustar o ponto exato no mapa.</p></details>
        <details><summary>Onde ficam meus favoritos?<Icon icon={Plus} /></summary><p>Os pratos marcados com coração aparecem em Favoritos e ficam salvos para você encontrar depois.</p></details>
      </div>
    </section>
  </motion.section>
}
...
function ProfilePanel({ modal, setModal, fullPage = false }) {
  ...
  const installApp = async () => {
    ...
    if (result?.outcome === 'manual-ios') {
      setInstallNotice('No Safari, toque em Compartilhar e depois em “Adicionar à Tela de Início”.')
      return
    }
    if (result?.outcome === 'unavailable') {
      setInstallNotice('No Chrome, abra o menu ⋮ e toque em “Instalar aplicativo”. A instalação completa fica disponível na versão publicada em HTTPS.')
      return
    }
    if (result?.outcome === 'accepted') setInstallNotice('TigreDelivery instalado. Você já pode abrir pela tela inicial.')
  }
  ...
  return <>
    ...
    {pwa.canInstall && <button className={styles.pwaInstall} onClick={installApp}><span className={styles.pwaInstallIcon}><Icon icon={Download} /></span><span><strong>Instalar TigreDelivery</strong><small>Acesse como aplicativo no seu celular</small></span><Icon icon={CaretRight} /></button>}
    {installNotice && <p className={styles.pwaInstallNotice} role="status">{installNotice}</p>}
...
export function CatalogPage() {
  ...
  useEffect(() => { document.title = 'TigreDelivery — O que vai ser hoje?' }, [])
  ...
  const heading = page === 'favorites' ? 'Favoritos' : page === 'offers' ? 'Ofertas' : page === 'orders' ? 'Seus pedidos' : page === 'help' ? 'Ajuda' : page === 'cart' ? 'Sua sacola' : page === 'payment' ? 'Pagamento' : page === 'product' ? selectedProduct.name : page === 'address' ? 'Endereços' : 'Explore o cardápio'
  ...
  const openProfile = () => setModal({ type: 'profile', navigate, openAddress: () => openAddress('home'), openFullProfile: openProfilePage, onHelp: () => navigate('help'), user: authUser, account, refresh: account.refresh, signOut })
  ...
  return <MotionConfig reducedMotion="user"><Tooltip.Provider delayDuration={500} skipDelayDuration={200}><div ref={chromeRef} className={`${styles.layout} relative isolate`}>
    <header className={styles.desktopHeader}>
      <a href="#catalogo" className={styles.desktopBrand} onClick={() => navigate('home')} aria-label="TigreDelivery, início"><span>Tigre<span>Delivery</span></span></a>
      <nav aria-label="Menu principal">{nav.map(item => <motion.button key={item.id} whileTap={tap} className={page === item.id ? styles.navActive : ''} onClick={() => navigate(item.id)} aria-current={page === item.id ? 'page' : undefined}><Icon icon={item.icon} weight={page === item.id ? 'fill' : 'regular'} /><span>{item.name}</span>{item.id === 'favorites' && favorites.length > 0 && <small>{favorites.length}</small>}</motion.button>)}</nav>
      <button className={styles.desktopHelp} onClick={() => navigate('help')}><Icon icon={Question} /><span>Ajuda</span></button>
    </header>

    <div className={`${styles.workspace} ${page === 'profile' ? styles.profileWorkspace : ''}`}><header className={styles.topbar}><motion.button whileTap={tap} className={`${styles.iconButton} ${styles.menuButton}`} onClick={openProfile} aria-label="Abrir perfil" aria-expanded={modal?.type === 'profile'}>{authUser?.user_metadata?.avatar_url ? <img className={styles.headerAvatar} src={authUser.user_metadata.avatar_url} alt="" referrerPolicy="no-referrer" /> : <Icon icon={User} />}</motion.button><motion.button whileTap={tap} className={styles.address} onClick={() => openAddress('home')}><span className={styles.addressCopy}><strong>{address?.label || 'Informe seu endereço'}</strong></span><Icon icon={CaretDown} /></motion.button><label className={styles.search}><Icon icon={MagnifyingGlass} /><input ref={searchInput} aria-label="Buscar pratos ou restaurantes" placeholder="Busque um prato ou restaurante" value={query} onChange={event => { setQuery(event.target.value); if (page === 'orders') setPage('home') }} />{query && <motion.button whileTap={tap} onClick={() => setQuery('')} aria-label="Limpar busca"><Icon icon={X} /></motion.button>}</label>{authUser ? <button className={`${styles.account} ${styles.accountAvatarButton}`} onClick={openProfile} aria-label="Abrir perfil">{account.profile?.avatar_url || authUser.user_metadata?.avatar_url ? <img src={account.profile?.avatar_url || authUser.user_metadata.avatar_url} alt="" referrerPolicy="no-referrer" /> : <Icon icon={User} />}</button> : <a className={styles.account} href="#entrar" aria-label="Entrar na sua conta"><Icon icon={User} /><span>Entrar</span></a>}<motion.button whileTap={tap} className={styles.mobileFilter} aria-label="Filtrar cardápio" aria-expanded={filters} onClick={() => { if (page === 'orders') setPage('menu'); setFilters(!filters); requestAnimationFrame(() => results.current?.scrollIntoView({ behavior: reducedMotion ? 'auto' : 'smooth', block: 'start' })) }}><Icon icon={SlidersHorizontal} />{(free || fast) && <i />}</motion.button><motion.button whileTap={{ scale: .92 }} className={styles.bag} onClick={() => navigate('cart')} aria-label={`Abrir sacola, ${count} itens`}><Icon icon={Bag} /><span>Sacola</span><AnimatePresence mode="popLayout" initial={false}><motion.b key={count} initial={{ scale: .45, rotate: -12 }} animate={{ scale: 1, rotate: 0 }} exit={{ scale: 1.3, opacity: 0 }} transition={spring}>{count}</motion.b></AnimatePresence></motion.button></header>
      <main className={styles.main}>{page === 'home' && !query && <MobileInstallBanner />}<AnimatePresence mode="wait" initial={false}><motion.div key={page} className={`${styles.pageContent} min-w-0`} initial={{ opacity: 0, y: reducedMotion ? 0 : 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: reducedMotion ? 0 : -6 }} transition={{ duration: .24, ease: [0.22, 1, 0.36, 1] }}>
        {!['profile', 'help'].includes(page) && (page !== 'home' || query) && <div className={`${styles.greeting} ${['cart', 'payment', 'product', 'address'].includes(page) ? styles.cartGreeting : ''}`}><h1>{heading}</h1></div>}
        {page === 'home' && !query && <div className={styles.discoveryDeck}>
          <HeroCarousel onBrowse={browse} />
          <CategoryCarousel value={category} onChange={setCategory} />
        </div>}
        {page === 'profile' ? <ProfilePage account={account} navigate={navigate} onBack={() => navigate('home')} onAddress={() => openAddress('profile')} onHelp={() => navigate('help')} signOut={signOut} />
