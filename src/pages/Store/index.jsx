...
function ProductPhotoEditor({ product, onChanged }) {
  const [editing, setEditing] = useState(false)
  const [url, setUrl] = useState(product.image_url || '')
  const [error, setError] = useState('')
  async function save(event) {
    event.preventDefault()
    if (url && !/^https:\/\//i.test(url.trim())) { setError('Use um link https://.'); return }
    const result = await supabase.from('store_products').update({ image_url: url.trim() || null }).eq('id', product.id).select('id').maybeSingle()
    if (result.error || !result.data) { setError('Não foi possível salvar a foto.'); return }
    setError('')
    setEditing(false)
    onChanged()
  }
  return editing ? <form className={styles.photoEditor} onSubmit={save}><input type="url" value={url} onChange={event => setUrl(event.target.value)} placeholder="https://link-da-foto.jpg" aria-label={`Foto de ${product.name}`} /><button type="submit">Salvar foto</button>{error && <small role="alert">{error}</small>}</form> : <button type="button" className={styles.photoEditButton} onClick={() => setEditing(true)}>{product.image_url ? 'Trocar foto' : 'Adicionar foto'}</button>
}
...
function MenuManager({ store, onChanged }) {
  ...
  const [notice, setNotice] = useState('')
  const [form, setForm] = useState({ name: '', description: '', image_url: '', price: '', category: '', prep_minutes: 25 })
  const load = useCallback(async () => { setLoading(true); const [catResult, productResult] = await Promise.all([supabase.from('store_categories').select('id, name, position, active').eq('store_id', store.id).order('position'), supabase.from('store_products').select('id, name, description, image_url, price, prep_minutes, available, category_id').eq('store_id', store.id).order('position')]); setCategoriesList(catResult.data || []); setProducts(productResult.data || []); setNotice(catResult.error || productResult.error ? 'Não foi possível carregar o cardápio.' : ''); setLoading(false) }, [store.id])
  useEffect(() => { const id = window.setTimeout(() => load(), 0); return () => window.clearTimeout(id) }, [load])
  async function addProduct(event) { event.preventDefault(); if (form.name.trim().length < 2 || Number(form.price) <= 0) { setNotice('Informe o nome e um preço válido.'); return } if (form.image_url && !/^https:\/\//i.test(form.image_url.trim())) { setNotice('Use um link de foto que comece com https://.'); return } setSaving(true); let categoryId = form.category || categoriesList[0]?.id; if (!categoryId) { const categoryResult = await supabase.from('store_categories').insert({ store_id: store.id, name: 'Principal', position: 0 }).select('id, name').single(); categoryId = categoryResult.data?.id; if (categoryResult.data) setCategoriesList([categoryResult.data]) } const { error } = await supabase.from('store_products').insert({ store_id: store.id, category_id: categoryId || null, name: form.name.trim(), description: form.description.trim() || null, image_url: form.image_url.trim() || null, price: Number(form.price), prep_minutes: Number(form.prep_minutes) || 25, position: products.length }); setSaving(false); if (error) { setNotice('Não foi possível adicionar o item.'); return } setForm({ name: '', description: '', image_url: '', price: '', category: categoryId || '', prep_minutes: 25 }); await load(); onChanged() }
  ...
  return <section className={styles.menuPanel}><header className={styles.sectionHeader}><div><h2>Cardápio</h2><p>Itens disponíveis para os clientes encontrarem sua loja.</p></div><button onClick={load} aria-label="Atualizar cardápio"><Icon icon={faRotate} />Atualizar</button></header><div className={styles.menuGrid}><form className={styles.productForm} onSubmit={addProduct}><div><span className={styles.formIcon}><Icon icon={faPlus} /></span><div><h3>Adicionar produto</h3><p>Comece com os itens mais pedidos.</p></div></div><label><span>Nome do produto</span><input value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} placeholder="Ex.: Smash com fritas" /></label><label><span>Descrição</span><textarea value={form.description} onChange={event => setForm({ ...form, description: event.target.value })} placeholder="Ingredientes e detalhes do item" /></label><label><span>Foto do prato <em>opcional</em></span><input type="url" value={form.image_url} onChange={event => setForm({ ...form, image_url: event.target.value })} placeholder="https://link-da-foto.jpg" /></label><div className={styles.formRow}><label><span>Preço</span><input type="number" min="0.01" step="0.01" value={form.price} onChange={event => setForm({ ...form, price: event.target.value })} placeholder="29,90" /></label><label><span>Preparo</span><select value={form.prep_minutes} onChange={event => setForm({ ...form, prep_minutes: event.target.value })}><option value="15">15 min</option><option value="25">25 min</option><option value="35">35 min</option><option value="45">45 min</option></select></label></div><label><span>Categoria</span><select value={form.category} onChange={event => setForm({ ...form, category: event.target.value })}><option value="">Principal</option>{categoriesList.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>{notice && <p className={styles.notice}>{notice}</p>}<button className={styles.addProduct} disabled={saving}>{saving ? 'Salvando…' : <>Adicionar ao cardápio<Icon icon={faArrowRight} /></>}</button></form><div className={styles.productList}>{loading ? <div className={styles.listLoading}><i /><i /><i /></div> : products.length ? products.map(item => <article key={item.id} className={!item.available ? styles.productUnavailable : ''}><span className={styles.productThumb}>{item.image_url ? <img src={item.image_url} alt="" /> : <Icon icon={faBowlFood} />}</span><div><strong>{item.name}</strong><small>{item.description || 'Sem descrição'} · {item.prep_minutes} min</small><b>{money(item.price)}</b><ProductPhotoEditor key={item.image_url || item.id} product={item} onChanged={load} /></div><button onClick={() => toggleProduct(item)} aria-label={`${item.available ? 'Pausar' : 'Ativar'} ${item.name}`}><Icon icon={item.available ? faToggleOn : faXmark} /></button><button onClick={() => removeProduct(item)} aria-label={`Remover ${item.name}`}><Icon icon={faTrash} /></button></article>) : <div className={styles.emptyMenu}><span><Icon icon={faBowlFood} /></span><h3>Seu cardápio começa aqui</h3><p>Adicione o primeiro produto para a loja ficar pronta.</p></div>}</div></div></section>
}

function OrdersPanel({ store, currentUserId }) {
  const [orders, setOrders] = useState([])
  const [presentations, setPresentations] = useState({})
  const [loading, setLoading] = useState(true)
  const load = useCallback(async () => { setLoading(true); const { data } = await supabase.from('store_orders').select('id, status, total, customer_note, created_at').eq('store_id', store.id).order('created_at', { ascending: false }).limit(30); setOrders(data || []); if (data?.length) setPresentations(await loadOrderPresentations(data)); setLoading(false) }, [store.id])
  useEffect(() => {
    const id = window.setTimeout(() => load(), 0)
    const poll = window.setInterval(() => { if (document.visibilityState === 'visible') load() }, 10000)
    const channel = supabase.channel(`store-orders-${store.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'store_orders', filter: `store_id=eq.${store.id}` }, load)
      .subscribe()
    return () => { window.clearTimeout(id); window.clearInterval(poll); supabase.removeChannel(channel) }
  }, [load, store.id])
  ...
  return <section className={styles.ordersPanel}><header className={styles.sectionHeader}><div><h2>Pedidos</h2><p>Itens, pessoas e conversa no mesmo lugar.</p></div><button onClick={load}><Icon icon={faRotate} />Atualizar</button></header>{loading ? <div className={styles.listLoading}><i /><i /><i /></div> : orders.length ? <div className={styles.orderList}>{orders.map(order => { const context = presentations[order.id]; const photo = orderPhoto(context); const active = !['cancelled', 'delivered'].includes(order.status); return <article key={order.id} className={styles.orderCard}>
    <div className={styles.orderCardMain}><span className={styles.orderIcon}>{photo ? <img src={photo} alt="" /> : <Icon icon={faReceipt} />}</span><div className={styles.orderCardTitle}><small>Pedido #{order.id.slice(0, 6).toUpperCase()} · {new Date(order.created_at).toLocaleString('pt-BR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</small><strong>{context?.item_name || 'Pedido da loja'}{context?.item_count > 1 ? ` + ${context.item_count - 1}` : ''}</strong>{order.customer_note && <p>“{order.customer_note}”</p>}</div><div className={styles.orderCardMeta}><b data-order-status={order.status}>{status[order.status]}</b><strong className={styles.orderTotal}>{money(order.total)}</strong></div></div>
    <div className={styles.orderCardPeople}><OrderIdentity name={context?.customer_name} photo={context?.customer_avatar_url} label="Cliente" />{context?.driver_id && <OrderIdentity name={context.driver_name} photo={context.driver_avatar_url} label="Motorista" />}</div>
    {active && <div className={styles.orderCardActions}><OrderChat orderId={order.id} currentUserId={currentUserId} otherLabel="cliente" orderLabel={store.name} context={context} />{['new', 'confirmed', 'preparing', 'ready', 'picked_up'].includes(order.status) && <button className={styles.orderAction} onClick={() => advance(order)}>{order.status === 'new' ? 'Aceitar' : order.status === 'preparing' ? 'Marcar pronto' : order.status === 'ready' ? 'Entregar ao motorista' : 'Avançar'}<Icon icon={faArrowRight} /></button>}</div>}
  </article> })}</div> : <div className={styles.ordersEmpty}><span><Icon icon={faBoxOpen} /></span><h3>A fila está tranquila</h3><p>Quando um cliente fizer um pedido, ele aparece aqui com o prazo e o valor.</p></div>}</section>
}
