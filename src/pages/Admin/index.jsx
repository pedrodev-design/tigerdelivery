...
const statusLabel = { pending: 'Em análise', approved: 'Aprovado', rejected: 'Recusado', suspended: 'Suspenso' }
const vehicleInfo = {
...
function AccessState({ account }) {
  ...
  if (loading) return <LoadingOverlay label="Quase lá" detail="Conferindo as permissões da sua conta." />
  return <main className={styles.accessPage}><div className={styles.accessCard}>
    <span><Icon icon={loading ? faRotate : faShieldHalved} /></span>
    <h1>{loading ? 'Abrindo painel' : account.user ? 'Acesso reservado' : 'Entre para continuar'}</h1>
    <p>{loading ? 'Conferindo as permissões da sua conta.' : account.user ? 'Somente administradores TigreFood podem revisar cadastros de entregadores.' : 'Use uma conta autorizada para acessar o painel administrativo.'}</p>
    {!loading && <a href={account.user ? '#catalogo' : '#entrar'}>{account.user ? 'Voltar ao TigreFood' : 'Entrar na conta'}<Icon icon={faChevronRight} /></a>}
  </div></main>
}
  ...
  return <motion.div className={styles.reviewOverlay} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}>
    <motion.aside className={styles.reviewPanel} initial={{ x: 30, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 24, opacity: 0 }} transition={{ duration: .22 }}>
      ...
      <div className={styles.identity}>
        ...
        <b data-status={application.status}>{statusLabel[application.status]}</b>
      </div>
      <section className={styles.reviewData}>
        <h3>Dados para conferência</h3>
        <dl>
          <div><dt><Icon icon={faIdCard} />CPF</dt><dd>{formatCpf(profile.cpf || '') || 'Não informado'}</dd></div>
          <div><dt><Icon icon={faPhone} />Celular</dt><dd>{formatPhone(application.phone)}</dd></div>
          <div><dt><Icon icon={faLocationDot} />Cidade</dt><dd>{application.city}</dd></div>
      ...
      <footer>
        <button className={styles.reject} disabled={saving} onClick={() => onReview('rejected', notes)}><Icon icon={faXmark} />Pedir correção</button>
        <button className={styles.approve} disabled={saving} onClick={() => onReview('approved', notes)}>{saving ? <i /> : <Icon icon={faCheck} />}Aprovar motorista</button>
      </footer>
    </motion.aside>
  </motion.div>
}
...
function StoreReviewPanel({ store, onClose, onReview, saving }) {
  ...
  return <motion.div className={styles.reviewOverlay} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}>
    <motion.aside className={styles.reviewPanel} initial={{ x: 30, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 24, opacity: 0 }} transition={{ duration: .22 }}>
      ...
      <footer><button className={styles.reject} disabled={saving} onClick={() => onReview('rejected', notes)}><Icon icon={faXmark} />Pedir correção</button><button className={styles.approve} disabled={saving} onClick={() => onReview('approved', notes)}>{saving ? <i /> : <Icon icon={faCheck} />}Aprovar loja</button></footer>
    </motion.aside>
  </motion.div>
}

function StoreQueue({ stores, loading, filter, setFilter, counts, notice, onRefresh, onSelect }) {
  const visible = stores.filter(item => filter === 'all' || (filter === 'rejected' ? ['rejected', 'suspended'].includes(item.status) : item.status === filter))
  return <section className={styles.queue}><header><div><h2>Fila de lojas</h2><p>Confira os dados antes de liberar uma operação no catálogo.</p></div><button onClick={onRefresh} aria-label="Atualizar"><Icon icon={faRotate} />Atualizar</button></header><div className={styles.filters}>{filters.map(item => <button key={item.id} onClick={() => setFilter(item.id)} className={filter === item.id ? styles.filterActive : ''}>{item.label}<span>{counts[item.id]}</span></button>)}</div>{notice && <p className={styles.notice}>{notice}</p>}{loading ? <div className={styles.skeleton}><i /><i /><i /></div> : visible.length ? <div className={styles.applicationList}>{visible.map(store => <button key={store.id} onClick={() => onSelect(store)}><span className={styles.person}><Icon icon={faStore} /></span><span className={styles.personInfo}><strong>{store.name}</strong><small>{store.category} · {store.city} · Enviada em {date(store.created_at)}</small></span><span className={styles.vehicle}><Icon icon={faBowlFood} />{store.is_open ? 'Aberta' : 'Fechada'}</span><b data-status={store.status}>{store.status === 'pending' ? 'Em análise' : store.status === 'approved' ? 'Aprovada' : 'Recusada'}</b><Icon icon={faChevronRight} /></button>)}</div> : <div className={styles.empty}><span><Icon icon={filter === 'pending' ? faCircleCheck : faStore} /></span><h3>{filter === 'pending' ? 'Fila em dia' : 'Nenhuma loja aqui'}</h3><p>{filter === 'pending' ? 'Não há novas lojas esperando análise.' : 'Os cadastros aparecem conforme o status selecionado.'}</p></div>}</section>
}
...
export function AdminPage() {
  ...
  useEffect(() => { document.title = 'Painel administrativo — TigreFood' }, [])
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select('id, user_id, phone, vehicle_type, vehicle_plate, city, status, review_notes, submitted_at, reviewed_at, profiles!driver_applications_user_id_fkey(full_name, cpf, avatar_url)')
  ...
  const loadStores = useCallback(async () => {
    ...
    const { data, error } = await supabase.from('stores').select('id, owner_id, name, category, cnpj, phone, city, address, description, status, review_notes, is_open, created_at').order('created_at', { ascending: false })
    setStores(data || [])
  ...
  const counts = useMemo(() => ({
    ...
    rejected: applications.filter(item => item.status === 'rejected' || item.status === 'suspended').length,
  }), [applications])
  const visible = applications.filter(item => filter === 'all' || (filter === 'rejected' ? ['rejected', 'suspended'].includes(item.status) : item.status === filter))
  const storeCounts = useMemo(() => ({ all: stores.length, pending: stores.filter(item => item.status === 'pending').length, approved: stores.filter(item => item.status === 'approved').length, rejected: stores.filter(item => ['rejected', 'suspended'].includes(item.status)).length }), [stores])

  async function review(status, notes) {
    setSaving(true)
    const { error } = await supabase.rpc('review_driver_application', { p_application_id: selected.id, p_status: status, p_notes: notes.trim() || null })
    setSaving(false)
    if (error) { setNotice(error.message?.includes('identity_verification_required') ? 'A verificação de identidade precisa estar conferida antes da aprovação.' : 'Não foi possível salvar a decisão. Tente novamente.'); return }
    setSelected(null)
    setNotice(status === 'approved' ? 'Motorista aprovado e acesso liberado.' : 'Cadastro devolvido para correção.')
    await load()
  }

  async function reviewStore(status, notes) {
    setSaving(true)
    const { error } = await supabase.rpc('review_store_application', { p_store_id: selectedStore.id, p_status: status, p_notes: notes.trim() || null })
    setSaving(false)
    if (error) { setNotice('Não foi possível salvar a decisão da loja. Tente novamente.'); return }
    setSelectedStore(null)
    setNotice(status === 'approved' ? 'Loja aprovada e acesso liberado.' : 'Cadastro da loja devolvido para correção.')
    await loadStores()
  }
  ...
  return <main className={styles.page}>
    <aside className={styles.sidebar}>
      <div className={styles.brand}><span><Icon icon={faShieldHalved} /></span><div><strong>TigreFood</strong><small>Operações</small></div></div>
      <nav><button className={section === 'drivers' ? styles.navActive : ''} onClick={() => setSection('drivers')}><Icon icon={faMotorcycle} />Motoristas</button><button className={section === 'stores' ? styles.navActive : ''} onClick={() => setSection('stores')}><Icon icon={faStore} />Lojas{storeCounts.pending > 0 && <small>{storeCounts.pending}</small>}</button></nav>
    ...
    <section className={styles.workspace}>
      ...
      {section === 'stores' ? <StoreQueue stores={stores} loading={storeLoading} filter={storeFilter} setFilter={setStoreFilter} counts={storeCounts} notice={notice} onRefresh={loadStores} onSelect={setSelectedStore} /> : <>
        ...
        <section className={styles.queue}>
          <header><div><h2>Fila de cadastros</h2><p>Confira os dados antes de liberar o acesso de motorista.</p></div><button onClick={load} aria-label="Atualizar"><Icon icon={faRotate} />Atualizar</button></header>
          <div className={styles.filters}>{filters.map(item => <button key={item.id} onClick={() => setFilter(item.id)} className={filter === item.id ? styles.filterActive : ''}>{item.label}<span>{counts[item.id]}</span></button>)}</div>
          {notice && <p className={styles.notice}>{notice}</p>}
          {loading ? <div className={styles.skeleton}><i /><i /><i /></div> : visible.length ? <div className={styles.applicationList}>
            {visible.map(application => { const profile = application.profiles || {}; const vehicle = vehicleInfo[application.vehicle_type] || vehicleInfo.motorcycle; return <button key={application.id} onClick={() => setSelected(application)}>
              <span className={styles.person}>{profile.avatar_url ? <img src={profile.avatar_url} alt="" referrerPolicy="no-referrer" /> : <Icon icon={faUser} />}</span>
              <span className={styles.personInfo}><strong>{profile.full_name || 'Nome não informado'}</strong><small>{application.city} · Enviado em {date(application.submitted_at)}</small></span>
              <span className={styles.vehicle}><Icon icon={vehicle.icon} />{vehicle.label}</span>
              <span className={`${styles.identityList} ${application.identity?.status === 'verified' ? styles.identityListVerified : ''}`}><Icon icon={faShieldHalved} />{application.identity?.status === 'verified' ? 'Identidade conferida' : application.identity?.status === 'pending' ? 'Identidade em análise' : 'Identidade pendente'}</span>
              <b data-status={application.status}>{statusLabel[application.status]}</b>
              <Icon icon={faChevronRight} />
            </button> })}
          </div> : <div className={styles.empty}><span><Icon icon={filter === 'pending' ? faCircleCheck : faUsers} /></span><h3>{filter === 'pending' ? 'Fila em dia' : 'Nenhum cadastro aqui'}</h3><p>{filter === 'pending' ? 'Não há novos entregadores esperando análise.' : 'Os cadastros aparecem aqui conforme o status selecionado.'}</p></div>}
        </section>
      </>}
      </div>
    </section>
